...
module.exports = function getDirectiveData(tsParsed, filePath, angularType) {
    ...
    (tsParsed.constructor.parameters || []).forEach(param => { // name, type, body
       ...
       export class Mock${param.type} extends ElementRef  {
          // constructor() { super(undefined); }
          nativeElement: any = {};
           constructor(nativeElementValue) {
             super(null);
             this.nativeElement = nativeElementValue;
               }
        }`);
            result.providers[param.type] = `{provide: ${param.type}, useValue: Mock${param.type}}`;
        } else if (param.type === 'Router') {
            result.imports[importLib] = result.imports[importLib] || [];
            result.imports[importLib].push(param.type);
            result.mocks[param.type] = reIndent(`
        const Mock${param.type} = jasmine.createSpyObj('Router', ['navigateByUrl']);
      `);
            result.providers[param.type] = `{provide: ${param.type}, useValue: Mock${param.type}}`;
        } else if (param.type === 'ConfirmDialogService') {
            result.imports[importLib] = result.imports[importLib] || [];
            result.imports[importLib].push(param.type);
            result.mocks[param.type] = reIndent(`
           export class Mock${param.type} {
